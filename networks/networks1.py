...
class ResNeXt(nn.Module):
    ...
    def __init__(self, baseWidth, cardinality, layers, num_classes):
        ...
        # corrected *****
        self.fc2 = nn.Linear(512 * block.expansion, self.num_classes)
        self.fc3 = nn.Linear(512 * block.expansion, self.num_classes)
        # corrected *****
    ...
    def forward(self, x):
        ...
        # corrected *****
        size = self.fc(x)
        centroid_x = self.fc2(x)
        centroid_y = self.fc3(x)
...
class MTNet(nn.Module):
    def __init__(self, nin, nout, nG=64):
        ...
        # task2
        self.conv8 = convBatch(nG * 4, nG * 8, stride=2)
        self.resconv1 = residualConv(nG * 8, nG * 8)
        self.resconv2 = residualConv(nG * 8, nG * 8)
        self.resconv3 = residualConv(nG * 8, nG * 16)
        self.resconv4 = residualConv(nG * 16, nG * 16)
        self.resconv5 = residualConv(nG * 16, nG * 16)
        self.relu = nn.ReLU(inplace=True)
        self.linear = nn.Linear(nG * 16 * 32 * 32, 3)  # size, centroid_x, centroid_y
    ...
    def forward(self, input):
        ...
        task1_y0 = self.deconv1(bridge)
        task1_y1 = self.deconv2(self.conv5(torch.cat((task1_y0, x2), dim=1)))
        task1_y2 = self.deconv3(self.conv6(torch.cat((task1_y1, x1), dim=1)))
        task1_y3 = self.conv7(torch.cat((task1_y2, x0), dim=1))
        task1_result = self.unetfinal(task1_y3)

        conv_x2 = self.conv8(x2)
        task2_y0 = self.resconv1(conv_x2)
        task2_y1 = self.resconv2(task2_y0)
        task2_y2 = self.resconv3(task2_y1)
        task2_y3 = self.resconv4(task2_y2)
        task2_y4 = self.resconv5(task2_y3)
        task2_y5 = task2_y4.view(task2_y4.size(0), -1)
        task2_y6 = self.linear(task2_y5)
        task2_result = self.softmax(task2_y6)
