...
class UNet_icl(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        icl_in_resolutions = (
        params['input_resolution'][1], params['input_resolution'][2], params['input_resolution'][3])
...
class InherentConsistent(nn.Module):
    def __init__(
        ...
        for i_layer in range(len(depths)):
            ...
            self.class_decoders.append(
                Class_Decoder(dim=in_chans[i_layer], input_resolution=ensure_tuple_rep(input_resolution[i_layer], 2),
            ...
            self.attn_convs0.append(
                SeparableConv2d(num_heads[i_layer], num_heads[i_layer], (3, 3), norm_layer=nn.BatchNorm2d,
                                relu_first=False))
            self.attn_convs1.append(
                nn.Conv2d(in_channels=num_heads[i_layer], out_channels=1, kernel_size=(1, 1), stride=(1, 1)))
            self.query_convs.append(
                nn.Conv1d(in_channels=in_chans[i_layer], out_channels=in_chans[i_layer] // 2, kernel_size=1, stride=1,
                          padding=0))
    ...
    def forward(self, feats, guided_Q=None, modal='labeled'):
        ...
        if modal == 'labeled':
            ...
            for i_layer in range(len(self.depth)):
                tok_feats = self.norm_layers[i_layer](
                    self.proj_layers[i_layer](feats[i_layer]).flatten(2).transpose(1, 2))
                updated_guided_Q, attn_map = self.class_decoders[i_layer](guided_Q[i_layer].expand(BS, -1, -1),
                                                                          tok_feats)
                bs, num_classes, num_heads, N_patch = attn_map.size()
                h, w = int(np.sqrt(N_patch)), int(np.sqrt(N_patch))
                attn_map = attn_map.contiguous().view(bs, num_classes, num_heads, h, w)
...
class Class_Decoder(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        # self.projection = nn.Linear(4096, 1024)
        self.norm3 = norm_layer(input_resolution[0] * input_resolution[1])

        self.mlp2 = MLP(in_features=input_resolution[0] * input_resolution[1],
                        hidden_features=input_resolution[0] * input_resolution[1], act_layer=act_layer, drop=drop)
    ...
    def forward(self, query, feat):
        ...
        # attn_projected = self.projection(attn)  # [2, 3, 8, 4096] -> [2, 3, 8, 1024]
...
# flops, params = profile(model, (dummy_input, dummy_input1))
# print('flops: ', flops, 'params: ', params)
# print('flops: %.2f M, params: %.2f M' % (flops / 1000000.0, params / 1000000.0))
# model = UNet_icl(in_chns=1, class_num=7)  # 输入通道1，类别7
# total_params = sum(p.numel() for p in model.parameters())
# print(f"实际参数量: {total_params / 1e6:.2f} M")  # 正常应为 10M~200M
