...
from networks.my_network import SAMHyDAS_Net
...
def net_factory(net_type="unet", in_chns=1, class_num=3):
    ...
    elif net_type == "unet_icl":
        net = UNet_icl(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "sam_v1":
        net = SAMHyDAS_Net(nin=in_chns, n_classes=class_num).cuda()
    # elif net_type == "mamba":
    #     net = Mamba_UNet(in_chans=in_chns, num_classes=class_num).cuda()
    elif net_type == "enet":
